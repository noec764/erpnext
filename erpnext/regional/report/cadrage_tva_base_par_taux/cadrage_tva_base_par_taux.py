...
class CadrageTVABase:
	def __init__(self, filters):
		...
		if (
			not getdate(self.fiscal_year.year_start_date)
			<= getdate(self.filters.date)
			<= getdate(self.fiscal_year.year_end_date)
		):
		...
		self.tax_accounts = frappe.get_all(
			"Account",
			filters={"is_group": 0, "account_type": "Tax", "company": self.filters.company},
			fields=["name", "account_number", "tax_rate"],
		)
		self.collection_accounts = [
			x.name
			for x in self.tax_accounts
	...
	def get_income_entries(self):
			...
			accounts.extend(
				frappe.get_all(
					...
					filters=dict(
						...
						voucher_no=("in", by_voucher_type[voucher_type]),
						company=self.filters.company,
					),
					pluck="account",
					distinct=1,
		...
		self.full_account_list = list(
			set([x for x in accounts if x in self.income_expense_accounts_list] + self.income_accounts_list)
		)

		income_gl_entries = self.get_income_gl_entries(
			["account", "SUM(credit- debit) as total"], "account"
		)
	...
	def get_income_expense_accounts(self):
		income_expense_accounts = frappe.get_all(
			"Account",
			filters=dict(
				account_type=("in", ("Income Account", "Expense Account")), company=self.filters.company
			),
		...
		self.income_accounts_list = [
			x.name for x in income_expense_accounts if x.account_type == "Income Account"
		]
		self.income_expense_accounts_number_dict = {
			x.name: x.account_number for x in income_expense_accounts
		}
		self.income_expense_accounts_name_dict = {
			x.name: x.account_name for x in income_expense_accounts
		}
	...
	def get_vat_account_per_item(self):
		tax_templates = {
			x.parent: (x.tax_type, x.tax_rate)
		...
		item_taxes = frappe.get_all(
			"Item Tax",
			filters={"item_tax_template": ("in", tax_templates.keys())},
	...
	def get_vat_split(self):
		...
		for voucher_type in gl_by_voucher_type:
			if voucher_type in ("Sales Invoice", "Purchase Invoice"):
				...
				fields.append(
					"income_account as account"
					if voucher_type == "Sales Invoice"
					else "expense_account as account"
				)
		...
		for gl_entry in income_gl_entries:
			gl_by_account[gl_entry.account][gl_entry.voucher_type][gl_entry.voucher_no] = [
				x
				for x in items
				if x.parenttype == gl_entry.voucher_type
				and x.parent == gl_entry.voucher_no
				and gl_entry.account == x.account
			]
			...
			for voucher_type in gl_by_account[account]:
				for voucher in gl_by_account[account][voucher_type]:
					if voucher_type in ("Sales Invoice", "Purchase Invoice"):
						...
						income_entries = {
							x.account: (flt(x.debit) - flt(x.credit)) * -1
							for x in gl_entries
							if x.account in self.income_accounts_list
						}
						...
						if len(tax_entries) == 1:
							...
							if tax_rates:
								for income_entry in income_entries:
									taxable_amount_by_account[income_entry][tax_rates[0].tax_rate] += flt(
										income_entries[income_entry], self.precision
									)
						else:
		...
		for data in self.data:
			...
			for tax_rate in self.tax_rates:
				...
				data.update({str(tax_rate): tax_amount})
		...
		for tax_rate in self.tax_rates:
			self.total_row.update({str(tax_rate): tax_total.get(tax_rate, 0.0)})
			...
			for g in group:
				...
				for account in accounts:
					date = (
						...
						if index == 1
						else add_days(self.fiscal_year.year_start_date, -1)
					)
					gl_entries = frappe.get_all(
						"GL Entry",
						filters=dict(
						...
						for gl_entry in gl_entries:
							for tax_account in tax_accounts:
								...
								if vat_entries and vat_entries[0].get("total"):
									...
									self.data[-1].update(
										{
											str(tax_account.tax_rate): flt(vat_entries[0].get("total"), self.precision)
											/ (tax_account.tax_rate or 1.0)
											* 100.0
											if vat_entries[0].get("total")
											else (flt(debit) - flt(credit))
											if index == 1
											else (flt(credit) - flt(debit))
										}
									)
	...
	def total_turnover(self):
		...
		for tax in self.tax_rates:
			...
			self.data[-1].update({str(tax): total})
	...
	def get_total_collected_vat(self):
		...
		for tax in self.tax_rates:
			...
			self.data[-1].update({str(tax): vat_amount})
	...
	def get_vat_in_books(self, balance_type="credit", label=None, filters=None):
		self.data.append(
			{"account": "", "is_total": False, "account_number": "", "account_name": label, "total": ""}
		)
		...
		for rate in self.tax_rates:
			accounts = [
				x.name
				for x in self.tax_accounts
				if x.tax_rate == rate and str(x.account_number).startswith("4457")
			]
			if accounts:
				filters.update(
					dict(
				...
				gl = frappe.get_all("GL Entry", filters=filters, fields=f"SUM({balance_type}) as total")

				if gl:
					self.data[-1].update({str(rate): flt(gl[0].get("total"), self.precision)})
	...
	def get_vat_in_accounts_total(self):
		...
		for tax in self.tax_rates:
			...
			self.data[-1].update({str(tax): flt(total)})
	...
	def get_difference(self):
		...
		for tax in self.tax_rates:
			difference = flt(self.total_amounts[tax][0], self.precision) - flt(
				self.total_amounts[tax][1], self.precision
			)
			self.data[-1].update({str(tax): difference})

	def control_values(self):
		for data in self.data:
			if data.get("total"):
				data["control"] = flt(
					flt(data.get("total")) - sum([flt(data.get(str(t))) for t in self.tax_rates]), self.precision
				)
	...
	def get_columns(self):
		self.columns = [
			...
			{"fieldname": "account_name", "label": _("Account Name"), "fieldtype": "Data", "width": 350},
		...
		for rate in self.tax_rates:
			self.columns.append(
				{"fieldname": str(rate), "label": f"{str(rate)} %", "fieldtype": "Currency", "width": 180}
			)

		self.columns.append(
			{"fieldname": "control", "label": _("Control"), "fieldtype": "Currency", "width": 180}
		)
