...
class BuyingController(StockController):
					...
					if self.docstatus in [0, 1] and not asset.get(field):
						...
						if asset.docstatus == 1 and delete_asset:
							frappe.throw(_('Cannot cancel this document as it is linked with submitted asset {0}. Please cancel it to continue.')
								.format(frappe.utils.get_link_to_form('Asset', asset.name, _('Asset'))))
	...
	def validate_schedule_date(self):
		...
		scheduled_dates = [d.schedule_date for d in self.get("items") if d.schedule_date is not None]
		if scheduled_dates:
			earliest_schedule_date = min(scheduled_dates)
