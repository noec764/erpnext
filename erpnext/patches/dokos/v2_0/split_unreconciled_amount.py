...
def execute():
	...
	for payment in frappe.get_all(
		...
		frappe.db.set_value(
			"Payment Entry",
			payment.name,
			"unreconciled_from_amount",
			payment.paid_amount if payment.payment_type in ("Pay", "Internal Transfer") else 0.0,
			update_modified=False,
		)
		frappe.db.set_value(
			"Payment Entry",
			payment.name,
			"unreconciled_to_amount",
			payment.paid_amount if payment.payment_type in ("Receive", "Internal Transfer") else 0.0,
			update_modified=False,
		)
	...
	for bank_transaction in bank_transactions:
		frappe.db.set_value(
			...
			"bank_account_head",
			accounts.get(bank_transaction.bank_account),
			update_modified=False,
		)
