...
def execute():
	...
	for payment_entry in frappe.get_all(
		...
		fields=["paid_amount", "received_amount", "payment_type", "name"],
	):
		frappe.db.set_value(
			"Payment Entry",
			payment_entry.name,
			"unreconciled_amount",
			flt(payment_entry.paid_amount)
			if payment_entry.payment_type == "Pay"
			else flt(payment_entry.received_amount),
		)

	for sales_invoice_payment in frappe.get_all(
		"Sales Invoice Payment",
		filters={"mode_of_payment": ["is", "set"], "clearance_date": ["is", "not set"]},
		fields=["sum(base_amount) as amount", "parent"],
		group_by="parent",
	):
		is_return = frappe.db.get_value("Sales Invoice", sales_invoice_payment.parent, "is_return")
		frappe.db.set_value(
			"Sales Invoice",
			sales_invoice_payment.parent,
			"unreconciled_amount",
	...
	for purchase_invoice in frappe.get_all(
		...
		fields=["base_paid_amount", "name", "is_return"],
	):
		frappe.db.set_value(
			"Purchase Invoice",
			purchase_invoice.name,
			"unreconciled_amount",
			(flt(purchase_invoice.base_paid_amount) * -1)
