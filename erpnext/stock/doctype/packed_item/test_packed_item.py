...
class TestPackedItem(FrappeTestCase):
	...
	def assertReturns(self, original, returned):
		...
		sort_function = lambda p: (p.parent_item, p.item_code, p.qty)  # noqa

		for sent, ret in zip(sorted(original, key=sort_function), sorted(returned, key=sort_function)):
			self.assertEqual(sent.item_code, ret.item_code)
			self.assertEqual(sent.parent_item, ret.parent_item)
			self.assertEqual(sent.qty, -1 * ret.qty)
