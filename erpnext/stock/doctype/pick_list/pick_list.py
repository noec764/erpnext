	...
	def before_save(self):
		...
		for location in self.get("locations"):
			if (
				...
				frappe.throw(_("Row #{}: item {} has been picked already.").format(location.idx, location.item_code))
	...
	def validate_picked_items(self):
		for item in self.locations:
			if self.scan_mode and item.picked_qty < item.stock_qty:
				frappe.throw(
					_("Row {0} picked quantity is less than the required quantity, additional {1} {2} required.").format(item.idx, item.stock_qty - item.picked_qty, item.stock_uom),
			...
			if len(item.serial_no.split("\n")) != item.picked_qty:
				frappe.throw(
					_("For item {0} at row {1}, count of serial numbers does not match with the picked quantity"
