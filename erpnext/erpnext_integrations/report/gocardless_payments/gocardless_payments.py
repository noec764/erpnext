...
def get_data(filters):
	...
	for payment in gocardless_payments:
		references = frappe.get_all(
			"Integration Request",
			filters={"service_id": payment.get("payment_id"), "integration_request_service": "GoCardless"},
			fields=["name", "reference_doctype", "reference_docname"],
		)
		payments = [
			x.reference_docname for x in references if x.reference_doctype == "Payment Entry"
		] + [
			x.name
			for x in frappe.get_all(
				"Payment Entry", filters={"reference_no": payment.get("payment_id"), "docstatus": 1}
			)
		]
		subscriptions = [
			x.subscription
			for x in frappe.get_all(
				"Payment Entry", filters={"name": ("in", list(set(payments)))}, fields=["subscription"]
			)
		]
		payment["dokos_payments"] = " ,".join(list(set(payments))) if payments else ""
		payment["dokos_payment_status"] = " ,".join(
			[
				_(x.status, context="Payment Entry")
				for x in frappe.get_all("Payment Entry", filters={"name": ("in", payments)}, fields=["status"])
			]
		)
		payment["dokos_subscription"] = (
			" ,".join(list(set(subscriptions))) if subscriptions and subscriptions[0] is not None else ""
		)

	if filters.get("subscription"):
		gocardless_payments = [
			x for x in gocardless_payments if filters.get("subscription") in x.get("dokos_subscription")
		]
...
def get_gocardless_payments(filters):
	...
	gocardless_gateways = frappe.get_all(
		"Payment Gateway", filters=query_filters, fields=["gateway_controller"]
	)
...
def get_columns(filters):
	return [
		...
		{"fieldname": "charge_date", "fieldtype": "Date", "label": _("Charge Date"), "width": 100},
		{"fieldname": "amount", "fieldtype": "Currency", "label": _("Amount"), "width": 150},
		{"fieldname": "description", "fieldtype": "Small Text", "label": _("Description"), "width": 250},
		{
...
def get_chart(filters, data):
	...
	if not df.empty:
		...
		aggregate = (
			df.groupby([df["date"].dt.strftime("%Y"), df["date"].dt.strftime("%B")])["amount"]
			.sum()
			.sort_values()
		)
	...
	return {
		"data": {
			...
			"datasets": [{"name": _("Amount"), "values": [result[x] for x in result]}],
		},
		"type": "line",
		"colors": ["rgb(3, 37, 80)"],
	}
