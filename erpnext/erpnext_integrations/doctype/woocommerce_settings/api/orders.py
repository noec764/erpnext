...
def sync_orders():
	...
	for woocommerce_order in woocommerce_orders:
		if str(woocommerce_order.get("id")) in excluded_ids:
			continue
		elif str(woocommerce_order.get("id")) in closed_orders and woocommerce_order.get("status") in (
			"failed",
			"refunded",
		):
			continue
...
def sync_order(wc_api, woocommerce_order):
	if woocommerce_order.get("customer_id") == 0 and not woocommerce_order.get("billing", {}).get(
		"email"
	):
		return
	...
	if customer:
		try:
			if frappe.db.exists(
				"Sales Order", dict(woocommerce_id=woocommerce_order.get("id"), docstatus=(">=", 1))
			):
...
def get_completed_and_excluded_orders():
	return frappe.get_all(
		"Sales Order",
		filters={"woocommerce_id": ("is", "set"), "status": "Completed"},
...
def get_closed_orders():
	return frappe.get_all(
		"Sales Order",
		filters={"woocommerce_id": ("is", "set"), "status": "Closed"},
		pluck="woocommerce_id",
	)
	...
	return frappe.get_doc(
		{
			...
			"delivery_date": add_days(
				woocommerce_order.get("date_created_gmt") or nowdate(), settings.delivery_after_days
			),
			"company": settings.company,
			"selling_price_list": settings.price_list,
			"ignore_pricing_rule": 1,
			"items": get_order_items(
				woocommerce_order, settings, woocommerce_order.get("date_created_gmt")
			),
			...
			"shipping_address_name": get_preferred_address(
				"Customer", customer.name, "is_shipping_address"
			),
...
def get_or_create_missing_item(settings, product):
	...
	if not item:
		item_doc = frappe.get_doc(
			get_simple_item(settings, {"name": product.get("name"), "categories": []})
		)
		try:
	...
	if sales_order:
		...
		if cint(settings.create_payments_and_sales_invoice):
			if woocommerce_order.get("status") == "refunded":
				...
				woocommerce_order.get("payment_method")
				and woocommerce_order.get("status") in DELIVERY_STATUSES
			):
...
def make_payment(woocommerce_order, sales_order):
	if (
		flt(sales_order.advance_paid) < flt(sales_order.grand_total)
		and woocommerce_order.get("transaction_id")
		and not frappe.get_all(
			"Payment Entry", dict(reference_no=woocommerce_order.get("transaction_id"))
		)
	):
		...
		if payment_entry.paid_amount:
			...
			payment_entry.reference_no = (
				woocommerce_order.get("transaction_id")
				or woocommerce_order.get("payment_method_title")
				or "WooCommerce Order"
			)
...
def refund_sales_order(settings, woocommerce_order, sales_order):
	sales_invoices = frappe.get_all(
		"Sales Invoice Item", filters={"sales_order": sales_order.name}, pluck="parent"
	)
