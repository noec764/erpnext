...
from frappe.utils import add_months, cint, formatdate, getdate, today
...
def add_institution(token, response):
	...
	if not frappe.db.exists("Bank", response["institution"]["name"]):
		try:
			...
			frappe.log_error(_("Plaid Link Error"))
...
def add_bank_accounts(response, bank, company):
	...
	for account in response.get("accounts"):
		...
		existing_bank_account = frappe.db.exists(
			"Bank Account", {"account_name": account["name"], "bank": bank["bank_name"]}
		)
...
def sync_transactions(bank, bank_account):
	...
	try:
		transactions = (
			get_transactions(bank=bank, bank_account=bank_account, start_date=start_date, end_date=end_date)
			or []
		)
		result = []
