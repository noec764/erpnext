...
def get_api_key(request):
	...
	if account:
		return frappe.db.get_value("GoCardless Settings", account, "webhooks_secret")
	else:
		...
		if len(gocardless_accounts) > 1:
			...
			return frappe.db.get_value("GoCardless Settings", gocardless_accounts[0].get("name"), "webhooks_secret")
