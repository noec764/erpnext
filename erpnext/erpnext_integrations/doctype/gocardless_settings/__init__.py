...
def webhooks():
	...
	for event in events:
		try:
			...
			frappe.enqueue(
				method="erpnext.erpnext_integrations.doctype.gocardless_settings.gocardless_settings.handle_webhooks",
				queue="long",
				timeout=600,
				is_async=True,
	...
	if account:
		...
		if len(gocardless_accounts) > 1:
			...
			return gocardless_accounts[0].get("name"), frappe.db.get_value(
				"GoCardless Settings", gocardless_accounts[0].get("name"), "webhooks_secret"
			)
