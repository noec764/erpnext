...
class GoCardlessPaymentWebhookHandler(WebhooksController):
	def __init__(self, **kwargs):
		...
		if self.gocardless_payment and self.metadata:
			frappe.db.set_value(
				self.integration_request.doctype,
				self.integration_request.name,
				"service_id",
				self.gocardless_payment.id,
	...
	def init_handler(self):
		self.gocardless_settings = frappe.get_doc(
			"GoCardless Settings", self.integration_request.get("payment_gateway_controller")
		)
	...
	def get_payout(self):
		self.gocardless_payout = self.data.get("links", {}).get("payout") or getattr(
			self.gocardless_payment.links, "payout", {}
		)
	...
	def add_fees_before_submission(self, payment_entry):
		...
		if self.gocardless_payout:
			...
			frappe.db.set_value(
				self.integration_request.doctype, self.integration_request.name, "output", output
			)
	...
	def get_base_amount(self, payout_items):
		paid_amount = sum(
			[
				...
				if (x.type == "payment_paid_out" and getattr(x.links, "payment") == self.gocardless_payment.id)
			]
		)
