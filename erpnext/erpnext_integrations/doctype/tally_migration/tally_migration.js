...
frappe.ui.form.on("Tally Migration", {
					...
					if (error_occurred) {
						frappe.msgprint({
							message: __("An error has occurred during {0}. Check {1} for more details",
								[
									repl("<a href='/app/Form/Tally Migration/%(tally_document)s' class='variant-click'>%(tally_document)s</a>", {
	...
	dokos_company: function (frm) {
		frappe.db.exists("Company", frm.doc.dokos_company).then(exists => {
			if (exists) {
				frappe.msgprint(
					__("Company {0} already exists. Continuing will overwrite the Company and Chart of Accounts", [frm.doc.dokos_company]),
				);
			}
		});
	},
