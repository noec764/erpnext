...
from erpnext.accounts.dashboard_chart_source.account_balance_timeline.account_balance_timeline import (
...
def execute(filters=None):
	dates = get_dates_from_timegrain(
		filters.get("date_range")[0], filters.get("date_range")[1], filters.get("period")
	)
...
def get_data(filters, dates):
	...
	for date in dates:
		output = [
			{
				f"{frappe.scrub(str(date))}_percent": (
					flt(x.get(frappe.scrub(str(date)))) / flt(x[f"{frappe.scrub(str(date))}_capacity"] or 1) * 100
				),
				**x,
			}
			for x in output
...
def get_calendar_capacity(filters, items, dates):
	for item in items:
		...
		for date in dates:
			...
			items[item][f"{frappe.scrub(str(date))}_capacity"] = sum(
				[daily_capacity[x] * weekdays[x[:3]] for x in daily_capacity if x[:3] in weekdays]
			)
			prev_date = date
