...
def validate_quantity_and_amount_for_suggestion(args, qty, amount, item_code, transaction_type):
	fieldname, msg = "", ""
	type_of_transaction = (
		_("purchase", context="Pricing Rule")
		if transaction_type == "buying"
		else _("sale", context="Pricing Rule")
	)
	...
	if fieldname:
		...
		).format(_(type_of_transaction), args.get(fieldname), bold(item_code), bold(args.title))
...
def apply_internal_priority(pricing_rules, field_set, args):
	...
	for field in field_set:
		if args.get(field):
			...
			filtered_rules = list(filter(lambda x: x.get(field) == args.get(field), pricing_rules))  # noqa
			if filtered_rules:
				break
...
def validate_coupon_code(coupon_name):
	...
	if coupon.valid_from:
		if coupon.valid_from > getdate(today()):
			frappe.throw(_("The coupon code's validity has not started"))
	elif coupon.valid_upto:
		if coupon.valid_upto < getdate(today()):
			frappe.throw(_("The coupon code's validity has expired"))
	...
	if coupon:
		if transaction_type == "used":
			if coupon.used < coupon.maximum_use:
				...
				frappe.throw(
					_("{0} has been use {1} times. Allowed quantity has been exceeded.").format(
