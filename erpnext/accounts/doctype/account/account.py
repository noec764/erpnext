...
class Account(NestedSet):
	...
	def autoname(self):
		self.name = get_account_autoname(
			self.account_number, self.account_name, self.company, self.root_type
		)
	...
	def check_if_child_acc_exists(self, doc):
		...
		info = frappe.db.get_value(
			"Account",
			{"account_name": doc.account_name, "account_number": doc.account_number},
			[
				"company",
				"account_currency",
				"is_group",
		...
		if dict_diff:
			frappe.throw(
				_(
					"Account {0} already exists in child company {1}. The following fields have different values, they should be same:<ul><li>{2}</li></ul>"
...
def get_account_autoname(account_number, account_name, company, root_type):
	...
	if (
		...
		and frappe.db.exists(
			"Account",
			{"account_number": account_number, "company": company.name, "root_type": ("!=", root_type)},
		)
	):
		suffix = _(root_type)
...
def update_account_number(
	...
	account_name,
	account_number=None,
	do_not_show_account_number_in_reports=False,
	from_descendant=False,
):
	account = frappe.get_cached_doc("Account", name)
	company, root_type = account.company, account.root_type

	if not (company and root_type):
		return
	...
	allow_independent_account_creation = frappe.get_cached_value(
		"Company", company, "allow_account_creation_against_child_company"
	)
		...
		for ancestor in ancestors:
			...
			if old_name:
				...
				message += _("To overrule this, enable '{0}' in company {1}").format(
					allow_child_account_creation, frappe.bold(company)
				)
	...
	frappe.db.set_value(
		"Account", name, "do_not_show_account_number", do_not_show_account_number_in_reports
	)
...
def sync_update_account_number_in_child(
	...
	for d in frappe.db.get_values(
		"Account",
		filters=filters,
		fieldname=["company", "name", "do_not_show_account_number"],
		as_dict=True,
	):
		update_account_number(
			d["name"], account_name, account_number, d["do_not_show_account_number"], from_descendant=True
		)
