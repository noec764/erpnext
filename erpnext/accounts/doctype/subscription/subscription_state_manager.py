...
class SubscriptionPeriod:
	...
	def get_current_invoice_start(self):
		if SubscriptionStateManager(self.subscription).is_trial() \
			...
			and self.subscription.get_doc_before_save().trial_period_end != self.subscription.trial_period_end:
			return max(getdate(self.subscription.start), add_days(getdate(self.subscription.trial_period_end), 1)) if self.subscription.trial_period_end else self.subscription.start
	...
	def get_next_invoice_date(self):
		if self.subscription.current_invoice_start and getdate(self.subscription.current_invoice_start) < getdate(self.subscription.current_invoice_start):
			return add_days(self.subscription.current_invoice_end, 1)
		elif self.subscription.current_invoice_start and getdate(self.subscription.current_invoice_start) == getdate(self.subscription.current_invoice_start):
