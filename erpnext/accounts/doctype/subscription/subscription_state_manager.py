...
class SubscriptionPeriod:
	...
	def get_current_documents(self, doctype):
		...
		documents = frappe.get_all(
			doctype,
			filters={"subscription": ("in", subscriptions), "docstatus": ["!=", 2]},
			or_filters={transaction_date: ["between", [self.start, self.end]], "name": ["in", events]},
			fields=fields,
		)
	...
	def get_next_invoice_date(self):
		if (
			...
			return add_to_date(
				add_days(self.subscription.trial_period_end, 1), **self.get_billing_cycle_data()
			)
...
class SubscriptionStateManager:
	...
	def set_status(self):
		...
		if status != self.subscription.status:
			...
			self.subscription.add_subscription_event(
				"Status updated", **{"previous_status": previous_status, "new_status": status}
			)

	def is_trial(self):
		return (
			getdate(self.subscription.trial_period_end) >= getdate(nowdate())
			if self.subscription.trial_period_start
			else False
		)
	...
	def is_billable(self):
		if getdate(self.subscription.cancellation_date) < getdate(nowdate()):
			...
			return (
				not (
					SubscriptionPeriod(
						self.subscription, start=previous_period[0].period_start, end=previous_period[0].period_end
	...
	def is_draft(self):
		...
		if self.subscription.generate_invoice_at_period_start:
			...
			previous_period = SubscriptionPeriod(self.subscription).get_previous_period()
			if previous_period:
				current_sales_invoices = SubscriptionPeriod(
					self.subscription, start=previous_period[0].period_start, end=previous_period[0].period_end
				).get_current_documents("Sales Invoice")
	...
	def is_payable(self):
		...
		if (
			current_sales_invoices
			and sum([flt(x.outstanding_amount) for x in current_sales_invoices]) > 0.0
			and not current_payment_requests
		):
			return True
	...
	def needs_sales_order(self):
		if self.subscription.create_sales_order and not SubscriptionPeriod(
			self.subscription
		).get_current_documents("Sales Order"):
			return True
		return False
