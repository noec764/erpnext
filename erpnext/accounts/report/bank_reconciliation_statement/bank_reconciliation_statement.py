...
def get_entries(filters):
	...
	return sorted(
		list(payment_entries) + list(journal_entries) + list(pos_entries) + list(purchase_invoices),
...
def get_entries_not_reflected(filters):
	...
	return sorted(
		list(je_entries) + list(pe_entries), key=lambda k: k["posting_date"] or getdate(nowdate())
	)
...
def get_bank_statement_row(filters, account_currency):
	balance = get_bank_transaction_balance_on(
		filters["bank_account"], getdate(filters["report_date"])
	)
