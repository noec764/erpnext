...
def validate_leave_allocation_against_leave_application(ledger):
	...
	leave_application_records = frappe.db.sql_list(f"""
		SELECT transaction_name
		FROM `tabLeave Ledger Entry`
		WHERE
			employee={frappe.db.escape(ledger.employee)}
			AND leave_type={frappe.db.escape(ledger.leave_type)}
			AND transaction_type='Leave Application'
			AND from_date>={ledger.from_date}
			AND to_date<={ledger.to_date}
	""")
