...
def get_context(context):
	...
	favorite_articles = get_favorite_articles_by_page_view(logged_in)
	if len(favorite_articles) < 6:
		...
		if not logged_in:
			filters.update({"login_required": 0})
	...
	context.help_article_list = get_help_article_list(logged_in)
	
def get_favorite_articles_by_page_view(logged_in):
	condition = "" if logged_in else " and t1.login_required=0"
...
def get_help_article_list(logged_in):
	help_article_list=[]
	...
	for category in category_list:
		...
		if not logged_in:
			filters.update({"login_required": 0})
		help_articles = frappe.get_all("Help Article", fields="*", filters=filters, order_by="modified desc", limit=5)
		if help_articles:
