...
def check_and_add_gateway(payment_request, gateway):
	if frappe.db.exists(
		"Payment Gateway Account", dict(payment_gateway=gateway, currency=payment_request.currency)
	):
...
def get_payment_details(link):
	...
	if payment_request.status in ["Paid", "Completed"] or outstanding:
		if flt(outstanding) == 0:
			...
			erpnext.get_company_currency(reference_document.get("company")) == payment_request.currency
			and flt(outstanding) < payment_request.grand_total
		):
		...
		if flt(advance) >= flt(total):
			...
			erpnext.get_company_currency(reference_document.get("company")) == payment_request.currency
			and flt(total) - (flt(advance) + flt(payment_request.grand_total)) < 0
		):
			error = _("The outstanding amount for this document is lower than the current payment request.")
	...
	return {
		...
		"formattedAmount": fmt_money(
			payment_request.grand_total, currency=payment_request.get("currency")
		),
		"error": error,
	}
