...
def make_tax_account_and_template(company, tax_data, template_name=None):
	for tax in tax_data.get("taxes"):
		tax["tax_account"] = make_tax_account(
			company, tax.get("account_name"), tax.get("rate"), tax.get("account_number")
		)
...
def make_tax_account(company, account_name, tax_rate, account_number=None):
	...
	if tax_group:
		try:
			...
			account = (
				"{0} - {1}".format(account_name, abbr)
				if not account_number
				else "{0} - {1} - {2}".format(account_number, account_name, abbr)
			)
...
def make_sales_and_purchase_tax_templates(company, tax_data, template_name=None):
	...
	if not sales_tax_template.get("taxes"):
		return
...
def make_item_tax_templates(company, tax_data, template_name=None):
	...
	if not item_tax_template.get("taxes"):
		return
...
def get_tax_account_group(company, account_number=None):
	if account_number:
		tax_groups = frappe.get_all(
			"Account",
			filters={
				"is_group": 1,
				"company": company,
				"account_number": ("in", (account_number[:-1], account_number[:-2])),
			},
			order_by="lft DESC",
		)
	...
	tax_group = frappe.db.get_value(
		"Account", {"account_name": "Duties and Taxes", "is_group": 1, "company": company}
	)
	if not tax_group:
		tax_group = frappe.db.get_value(
			"Account", {"is_group": 1, "root_type": "Liability", "account_type": "Tax", "company": company}
		)
...
def get_country_wise_tax(country):
	data = {}
	with open(
		os.path.join(os.path.dirname(__file__), "..", "data", "country_wise_tax.json")
	) as countrywise_tax:
