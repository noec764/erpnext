...
class TestItemReview(FrappeTestCase):
	...
	def tearDown(self):
		...
		frappe.get_cached_doc("Website Item", {"item_code": "Test Mobile Phone"}).delete(force=True)
	...
	def test_add_and_get_item_reviews_from_customer(self):
		...
		try:
			add_item_review(web_item, "Great Product", 3 / 5, "Would recommend this product")
		...
		self.assertEqual(review_data.average_rating, 3 / 5)
